...
grid_config_bevformer={
    'x': [-40, 40, 0.4],
    'y': [-40, 40, 0.4],
    'z': [-1, 5.4, 1.6], # 4 sample points
}
bev_h_ = 200 # correspond to y axis
bev_w_ = 200 # correspond to x axis

_dim_ = 256
_pos_dim_ = numC_Trans // 2
...
model = dict(
    ...
    img_backbone=dict(
        ...
        depth=50,
        num_stages=4,
        out_indices=(0, 1, 2, 3), # 2^2, 2^3, 2^4, 2^5
        frozen_stages=-1,
    ...
    img_neck=dict(
        ...
        num_outs=1,
        start_level=0,
        out_ids=[0]), # CenterPoint, L3 Feature
    img_pvsnet=dict(
        type='PVSNet',
        channels=_dim_), # 
...
train_pipeline = [
    ...
    dict(type='PointToMultiViewSemantic', downsample=2**3, grid_config=grid_config),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_depth', 'gt_semantic','voxel_semantics',
