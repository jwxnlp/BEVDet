...
class BEVFormer(BaseModule):
    ...
    def init_layers(self):
        ...
        self.cams_embeds = nn.Parameter(
            torch.Tensor(self.num_cams, self.embed_dims)) # [N_view, E]
    ...
    def forward(
            ...
            bev_pos=None, # [B, 2*N_feats, bev_h, bev_w]
            cam_params=None, #
        ...
        for lvl, feat in enumerate(mlvl_feats):
            ...
            spatial_shape = (h, w)
            feat = feat.flatten(3).permute(1, 0, 3, 2) # [N_view, B, H_L4*W_L4, con_C]
        ...
        # [N_view, B, N_MLVL=H_L4*W_L4+..., con_C]
        feat_flatten = torch.cat(feat_flatten, 2)
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=bev_pos.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros(
            (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        # [N_view, N_MLVL, B, con_C]
        ...
        bev_embed = self.encoder(
            ...
            bev_h=bev_h,
            bev_w=bev_w,
            bev_pos=bev_pos, # [bev_h * bev_w, B, 2*_pos_dim_]
